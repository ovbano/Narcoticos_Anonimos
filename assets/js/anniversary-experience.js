(() => {
  ...
  function setupCalendar() {
    ...
    root.innerHTML=`<div class="ann-toolbar"><div class="ann-month-nav"><button type="button" class="ann-button" data-move="-1" aria-label="Mes anterior">←</button><h3 id="ann-month-label"></h3><button type="button" class="ann-button" data-move="1" aria-label="Mes siguiente">→</button></div><button type="button" class="ann-button" id="ann-today">Este mes</button><label class="ann-search"><i class="bi bi-search" aria-hidden="true"></i><input type="search" id="ann-search" aria-label="Buscar compañero en el mes seleccionado" placeholder="Buscar en este mes…"></label></div><div class="ann-explorer-layout"><section class="ann-month-sheet" aria-labelledby="ann-month-label"><div class="ann-week" aria-hidden="true">${['L','M','X','J','V','S','D'].map(x=>`<span>${x}</span>`).join('')}</div><div class="ann-days" id="ann-days"></div><p class="ann-legend"><span>● Aniversario</span><span>● Celebración</span><span>○ Hoy</span></p></section><section class="ann-agenda"><div class="ann-agenda-heading"><h4 id="ann-agenda-title">Para acompañarnos</h4><button class="ann-button" type="button" id="ann-clear" hidden>Ver todo el mes</button></div><p id="ann-count" role="status"></p><div id="ann-agenda-list"></div></section></div>`;
  ...
  function renderDetail() {
    ...
    root.innerHTML=`<section class="ann-invitation-hero"><span class="ann-kicker">GRUPO AMIGOS VERDADEROS · ${year}</span><div class="ann-invitation-emblem" aria-hidden="true"><i class="bi bi-stars"></i></div><span class="ann-invitation-status">${esc(status)}</span><h1>${esc(item.name)}</h1><p>${event.milestone?`<strong>${event.milestone}</strong> ${event.milestone===1?'año':'años'} de recuperación`:'Una historia de recuperación'}</p><span>Nos acompañamos, un día a la vez.</span></section>
      <div class="ann-detail-grid"><section class="ann-detail-panel"><span class="ann-kicker">UNA FECHA PARA COMPARTIR</span><h2>${esc(fmt(event.date))}</h2><dl class="ann-facts"><div><dt>Fecha original del aniversario</dt><dd>${esc(fmt(event.original))}</dd></div><div><dt>Celebración</dt><dd>${celebration?'Fecha programada por el grupo':'Sin fecha adicional programada'}</dd></div><div><dt>Lugar</dt><dd>${celebration?.confirmed?esc(celebration.location || 'Ubicación confirmada en el mapa'):'Por confirmar'}</dd></div></dl><p class="ann-small">Fechas según Ecuador. Consulta al grupo para confirmar la hora.</p><div class="ann-actions"><button class="ann-button ann-primary" type="button" data-share-url="${esc(link(event))}" data-share-name="${esc(item.name)}"><i class="bi bi-share"></i> Compartir invitación</button><a class="ann-button" href="https://wa.me/?text=${encodeURIComponent(`Aniversario de ${item.name} · ${fmt(event.date)}\n${link(event)}`)}" target="_blank" rel="noopener">WhatsApp</a></div></section>
      <section class="ann-detail-panel ann-message"><i class="bi bi-chat-heart" aria-hidden="true"></i><span class="ann-kicker">${celebration?.message?'MENSAJE DE INVITACIÓN':'PARA ACOMPAÑARNOS'}</span><h2>Cada día cuenta</h2><p class="ann-message-text">${esc(celebration?.message || 'Acompañar una historia de recuperación es recordar que no estamos solos. Compartamos este día con respeto, gratitud y esperanza.')}</p><small>Grupo Amigos Verdaderos</small></section></div>
